...
- Fashion/clothing simulation (stockings, garters, corsets, belts)
- Stylized characters requiring exaggerated flesh effects
- Any scenario with soft-body / rigid-body interaction

<!-- TODO: Add hero image showing before/after comparison -->
<!-- ![Hero Image](Images/FleshRing_Hero.png) -->
...
| **7** | In Toolbar → click **"Generate Bake"** button (critical!) |
| **8** | Add `UFleshRingComponent` to your character Blueprint → set **Flesh Ring Asset** |

> **Critical:** Step 7 (Generate Bake) is mandatory. Without baking, deformations visible in the editor will NOT appear at runtime. The baked mesh is stored inside the FleshRing Asset.
...
<!-- TODO: Add Quick Start screenshots -->
...
**[Step 7 - Generate Bake]**![Step 7 - Generate Bake](Images/QuickStart_05_GenerateBake.png)
...
| Pre-attached ring on skeletal mesh | **Virtual Ring** | No separate mesh needed |
...
| **BakedMesh** | Ring assets must be baked before runtime use (`Generate Bake` in editor) |
...
|----------|---------------------|
| Stocking on skin (want both to compress) | Enable both Skin and Stocking in mask |
...
​	![](C:\Users\Jungle\Perforce\hosi_WorkSpace\FleshRing\Plugins\FleshRingPlugin\Docs\Images\LayerType.png)
...
| Effect Range Mode | `EffectRangeMode` | Enum | Mesh Based | — | Mesh Based / Virtual Ring / Virtual Band |
...
#### Effect Range Mode (`EFleshRingEffectRangeMode`)
...
### Blueprint Example: Runtime Asset Swap

```
// BeginPlay: BakedMesh is automatically applied if FleshRingAsset is set
// No manual function call required

Event OnEquipmentChanged(NewEquipment)
    └─► Switch on Equipment Type
        ├─► Stockings → SwapFleshRingAsset(StockingAsset)
        ├─► Garter → SwapFleshRingAsset(GarterAsset)
        └─► None → SwapFleshRingAsset(nullptr)  // Restores original mesh
```
...
<!-- TODO: Add Blueprint node screenshot for Modular Character API -->
...
| **BakedMesh Requirement** | Ring effects only work with baked deformation. Call `Generate Bake` in the Asset Editor first. |
...
2. **Use Virtual Ring/Band** modes when possible (faster SDF generation)
...
A: You need to click **"Generate Bake"** in the Asset Editor. Editor preview and runtime use different systems—the baked mesh is required for runtime.
...
**Q: Does this work with morph targets/blend shapes?**
A: Yes. The baked mesh preserves morph target compatibility. Deformation is applied on top of morph target results.
...
A: Enable **Edge Length Preservation** and increase iterations. Also try **Volume Preserving** smoothing type if using high smoothing iterations.
...
- Option 1: Enable only Skin in the mask (stocking won't deform, follows skin surface)
- Option 2: Enable only Stocking in the mask (skin stays in place)
- Option 3: Use separate Rings with different layer masks
...
|-------|-------|----------|
| No deformation at runtime | Bake not generated | Click "Generate Bake" in Asset Editor |
...
| Slow editor preview | High SDF resolution or iterations | Reduce SDF to 32, smoothing to 10 |
...
**Features:**
- GPU-accelerated multi-stage deformation pipeline (9 stages)
- SDF-based influence calculation with OBB optimization
- Tightness, Bulge, Laplacian Smoothing, PBD Edge Constraints
- Radial Smoothing for uniform radius distribution
- Deformation Spread (Heat Propagation) for gradual falloff
- Material Layer System with AffectedLayerMask control
- Layer types: Skin, Stocking, Underwear, Outerwear, Other, Exclude
- Multiple Ring support per asset
- Influence Modes: Mesh Based, Virtual Ring, Virtual Band
- Mesh Subdivision for affected areas
- Normal Recomputation (Geometric / Surface Rotation)
- Displacement-based normal blending
- Skinned Ring Mesh for twist bone support
- Modular Skeletal Mesh support
- Full Blueprint & C++ API
- Dedicated Asset Editor with 3D preview viewport
- Interactive transform gizmos
- Skeleton bone tree browser
- Bake-based workflow for zero-runtime-cost deployment

**Supported Platforms:**
- Windows (DirectX 11/12)

**Known Limitations:**
- macOS/Linux not yet supported (Metal/Vulkan compute shader port planned)
- Maximum 16 Rings per Asset (engine limitation)
...
<!--
TODO: Before FAB submission, add your actual support channels:
- Discord server link
- YouTube tutorial playlist
- GitHub issues page (if public)
-->
...
5. Relevant log output (`LogFleshRing`)
...
You are free to use, modify, and distribute this plugin for both personal and commercial projects without attribution requirement.

See the `LICENSE` file included with the plugin for full legal terms.

### Third-Party Notices

This plugin uses the following open-source components:
- None (fully original implementation)
...
<!--
================================================================================
DOCUMENTATION CHECKLIST FOR FAB SUBMISSION (v3)
================================================================================
[x] Contact/Support information with specific email/Discord
[x] Engine version compatibility clearly stated
[x] Installation steps with folder structure
[x] Quick Start guide (5-minute goal)
[x] Parameter reference with CORRECT names and defaults
[x] Pipeline diagram with CORRECT order (Editor vs Runtime)
[x] Material Layer System section (AffectedLayerMask, Exclude)
[x] Use case tutorials with recommended settings
[x] Code examples (C++ and Blueprint)
[x] Modular Character Support section (Skeletal Merging, Leader Pose workflows)
[x] Modular Character API (UFleshRingModularLibrary full reference)
[x] Troubleshooting section with layer crossing fix
[x] FAQ section
[x] Version history / changelog
[x] License information
[x] Dependencies listed
[x] Shader list (22 files)
[x] All debug visualization options
[ ] Screenshots/GIFs (placeholders marked with TODO comments)
[ ] Video tutorial links (placeholder)
================================================================================
CHANGES FROM v2:
- Fixed parameter names (TightnessStrength, BulgeIntensity, SmoothingLambda, etc.)
- Fixed default values (TightnessStrength=1.5, SmoothingIterations=20, PBDIterations=5)
- Corrected pipeline order (SDF at init, Skinning at runtime end)
- Added Material Layer System section
- Added Exclude layer type documentation
- Added AffectedLayerMask workflow explanation
- Removed non-existent RefreshDeformation() API
- Added actual API functions (UpdateSDF, RefreshRingMeshes, UpdateRingTransforms)
- Added Subdivision settings
- Added Normal Recomputation settings
- Added SmoothingVolumeMode parameter
- Added all Bulge falloff types (WendlandC2, etc.)
- Updated shader count (22 files)
- Corrected header count (55 total: 44 Runtime + 11 Editor)
- Added all debug visualization options
- Added FAQ about skin/stocking crossing
================================================================================
CHANGES FROM v3 initial:
- Added Radial Smoothing parameters (bEnableRadialSmoothing, RadialBlendStrength, RadialSliceHeight)
- Added Deformation Spread / Heat Propagation section (bEnableHeatPropagation, iterations, lambda, seeds)
- Added BulgeRadialTaper parameter
- Added Effect Bounds section (SDFBoundsExpandX/Y)
- Added Displacement-Based Normal Blending (bEnableDisplacementBlending, MaxDisplacementForBlend)
- Added bEnableTangentRecompute parameter
- Updated pipeline diagram with Radial Smooth CS and Heat Propagate CS stages
- Updated Compute Shader Pipeline table (9 stages)
- Added SetTargetMesh() API function
- Corrected header count (56 total: 45 Runtime + 11 Editor)
- Added missing shaders to list (HeatPropagateCS, RadialSmoothCS, TangentRelaxCS)
- Added Skinned Ring Mesh section (bGenerateSkinnedRingMesh, RingSkinSamplingRadius)
================================================================================
CHANGES FROM v3 cross-verification (2026-01-31):
- Fixed Bulge Falloff types: removed Gaussian/WendlandC4/SmoothStep, kept Linear/Quadratic/Hermite/WendlandC2/Smootherstep
- Fixed Laplacian types: removed Cotan, kept Standard/Volume Preserving
- Fixed AffectedLayerMask default: All → Skin | Other
- Fixed Transform parameters: RelativeLocation/Rotation/Scale → MeshOffset/MeshEulerRotation/MeshScale
- Fixed MaxSubdivisionLevel range: 1-4 → 1-6
- Fixed BandEulerRotation default: (0,0,0) → (-90,0,0)
- Fixed TightnessStrength range: 0.0~5.0 → 0.0~3.0
- Fixed SmoothingIterations range: 0~50 → 1~30
- Fixed MaxSmoothingHops range: 1~30 → 1~20
- Fixed RingThickness range: 0.1~50 → 0.1~20
- Fixed BulgeAxialRange range: 0.1~50 → 1.0~8.0
- Fixed BulgeRadialRange range: 0.1~20 → 1.0~3.0
- Fixed PBDIterations range: 1~20 → 1~100
- Added missing parameters: UpperBulgeStrength, LowerBulgeStrength, BulgeRadialRatio, PBDTolerance, bPBDAnchorAffectedVertices, HopFalloffType
- NEW: Dual-column parameter tables (UI Label + Property) for artist/programmer accessibility
- Updated all parameter references to use consistent UI Labels throughout document
- Expanded Feature Deep-Dive section 1: "Influence Calculation (3 Modes)" with Mesh Based/Virtual Ring/Virtual Band details
- Added Mode Selection Guide table
- Fixed Engine version consistency (5.7)
================================================================================
CHANGES FROM v3.1 (2026-02-01) - Modular Character Documentation:
- NEW: "Modular Character Support" subsection in Core Features (~45 lines)
  - Workflow comparison table (Skeletal Merging vs Leader Pose)
  - Skeletal Merging workflow explanation
  - Leader Pose workflow explanation
  - Common requirements table
- NEW: "Modular Character API" subsection in Blueprint & C++ API (~120 lines)
  - UFleshRingModularLibrary class documentation
  - RebuildMergedMesh() full API reference with FFleshRingModularPart, FFleshRingMergeOutput
  - SwapModularRingAsset() full API reference
  - SwapModularPartMesh() full API reference
  - FFleshRingModularResult enum documentation
  - C++ example: Leader Pose setup with runtime swap
  - C++ example: Skeletal Merging workflow
  - Important notes table (error handling, performance considerations)
  ================================================================================
  -->
